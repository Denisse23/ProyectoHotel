<%-- 
    ...
    Created on : 4/12/2016, 08:48:43 PM
    Author     : Denisse
--%>
    ...
    <body>
        ...
        <%
            try {
                ...
                db.prepare(sql);
                db.query.setString(1, request.getParameter("select-id-agregar-categoria"));
                db.query.execute();
        ...
        <%
            try {
                Dba db = new Dba(application.getRealPath("Hotel.mdb"));
                db.conectar();
                String sql = "insert into Habitacion (IdHabitacion, IdCategoria) values"
                        + "(?,?)";
                db.prepare(sql);
                db.query.setString(1, request.getParameter("text-id-nuevo-habitacion"));
                db.query.setString(2, request.getParameter("select-id-agregar-categoria"));
                int contador = db.query.executeUpdate();
                if (contador == 1) {
                    DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                    Date date = new Date();
                    String sql1 = "insert into Bitacora (Accion, Tabla, Usuario, Fecha, Rol) values('insert','Habitacion'"
                            + ",?,?,?)";
                    db.prepare(sql1);
                    ...
                    db.query.executeUpdate();
                    db.commit();
                    out.print("<script>alert('La Habitación se agregó correctamente');</script>");
                }
                db.commit();
                db.desconectar();

            } catch (Exception e) {
                out.print("<script>alert('Error: El Id de Habitación no puede ser duplicado');</script>");
            }
